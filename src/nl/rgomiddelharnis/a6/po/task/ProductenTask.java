...
/**
 * Deze class gebruikt een achtergrond thread om de producten op te halen.
 * 
 * @author Sven Dubbeld <sven.dubbeld1@gmail.com>
 * @see AsyncTask
 */
public class ProductenTask extends AsyncTask<List<NameValuePair>, Void, JSONObject> {
     ...
     */
    private static final String TAG = "ProductenTask";
    ...
    /**
     * Start een nieuwe <code>ProductenTask</code> om de producten op te halen.
     * 
     * @param context {@link Context} De Activity die de Task start.
     */
    public ProductenTask(Context context) {
        ...
        if (error == null) {
            ...
            try {
                ...
                if (Integer.parseInt(res) == 1) {
                    ...
                    // Haal 'producten' object op
                    JSONObject json_producten = result.getJSONObject("producten");
                    Iterator<?> producten = json_producten.keys();

                    // Voeg producten toe aan database
                    mDb.leegProducten();
                    while (producten.hasNext()) {
                        String id = producten.next().toString();
